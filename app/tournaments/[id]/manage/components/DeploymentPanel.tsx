    ...
    <div className="bg-foreground/5 border border-foreground/5 p-8 rounded-[2.5rem] space-y-8">
      ...
      <div className="space-y-4">
        ...
        <select value={selectedUserId} onChange={e => setSelectedUserId(e.target.value)} className={`w-full ${inputCls}`}>
          ...
          {available.map(u => (
            <option key={u.id} value={u.id}>{((u as any).username || "Unknown").toUpperCase()}</option>
          ))}
        </select>
      ...
      {remaining > 0 && (
        <div className="space-y-4 pt-4 border-t border-foreground/10">
          <p className="text-[9px] font-black text-foreground/40 uppercase tracking-widest font-poppins ml-1">Batch Add Guests</p>
          <div className="flex gap-2">
