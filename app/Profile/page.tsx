...
function ProfileContent() {
  ...
  useEffect(() => {
    // If there's no ID in the URL at all, we could default to the current user
    // or just fetch me and if it succeeds use my ID, else redirect to Auth.
    ...
    const fetchProfileData = async () => {
      try {
        ...
        // If no profileId is provided in URL, default to my own profile if logged in
        ...
        // 3. Fetch leaderboard stats regardless, to populate the Performance Stats
        ...
        if (lbRes.ok) {
          ...
          if (targetUserStats) {
            ...
            // If viewing someone else, we construct their profile from leaderboard data
            if (!isMe) {
              setUser({
             ...
             // Not me, and not on leaderboard -> Can't view profile
             setUser(null);
          }
        }
      } catch (err) {
    ...
    <div className="min-h-screen w-full bg-background font-questrial flex flex-col overflow-x-hidden">
      ...
      <main className="flex-1 flex flex-col items-center px-4 py-12 sm:px-8 w-full">
        <div className="w-full max-w-7xl">
          {/* Top Section: Profile Info (Left) & Featured Stats (Right) */}
          <div className="flex flex-col lg:flex-row gap-8 mb-10">
            
            {/* Profile Info (Takes most of the width) */}
          ...
          {/* Bottom Section: Match History & Stats Grid (1/2 and 1/2) */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {/* Match History (Left half) */}
            <div className="bg-gradient-to-br from-foreground/5 to-transparent border border-primary/10 rounded-none p-8 md:p-10 flex flex-col min-h-[400px]">
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center bg-background/50 rounded-none border border-dashed border-primary/20 p-8">
                ...
                <p className="text-foreground/30 text-xs font-black uppercase tracking-widest leading-relaxed max-w-xs">
                  Awaiting telemetry data.<br/>Play matches to populate history.
                </p>
              </div>
            </div>

            {/* Performance Stats (Right half) */}
            <div className="bg-gradient-to-bl from-foreground/5 to-transparent border border-primary/10 rounded-none p-8 md:p-10 flex flex-col min-h-[400px]">
              ...
              {stats ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center bg-background/50 rounded-none border border-dashed border-primary/20 p-8">
                  ...
                  <p className="text-foreground/30 text-xs font-black uppercase tracking-widest leading-relaxed max-w-xs">
                    Insufficient combat data to generate performance matrix.
                  </p>
                </div>
              )}
            </div>
