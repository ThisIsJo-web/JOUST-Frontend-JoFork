...
export default function ControlRoomHeader({ tournament, tournamentId, onBack, onOpenArena, onGenerateBracket, onStartTournament, onViewBracket }: Props) {
  return (
    <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12 border-b border-foreground/5 pb-12">
      ...
      <div className="flex flex-wrap gap-4 w-full md:w-auto">
        ...
        {tournament.status === "OPEN" && (
          <>
            <button onClick={onGenerateBracket} className="flex-1 md:px-12 py-5 border-2 border-foreground text-foreground font-black text-xs uppercase tracking-[0.2em] rounded-2xl hover:bg-foreground hover:text-background transition-all font-poppins">
              {(tournament.rounds?.length ?? 0) > 0 ? "Regenerate Preview" : "Generate Preview"}
            </button>
