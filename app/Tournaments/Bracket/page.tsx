...
function BracketViewContent() {
  ...
  const handleAddGuest = async () => {
    ...
    const res = await authenticatedFetch(API_ENDPOINTS.TOURNAMENTS.JOIN_GUEST(tournamentId!), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ guestName: guestUsername }) });
    const data = await safeJson(res);
  ...
  const handleBatchAddGuests = async () => {
    ...
    if (countToAdd <= 0) { addLog("ERROR", "ARENA AT CAPACITY"); return; }
    if (!confirm(`Initialize bulk deployment of ${countToAdd} guests?`)) return;
    addLog("BATCH_INIT", `SUMMONING ${countToAdd} COMBATANTS...`);
    for (let i = 0; i < countToAdd; i++) {
      await authenticatedFetch(API_ENDPOINTS.TOURNAMENTS.JOIN_GUEST(tournamentId!), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ guestName: randomGuestName() }) });
    }
    setBatchGuestCount(""); await fetchTournamentData();
  };
  ...
  return (
    <div className="min-h-screen w-full bg-background font-questrial flex flex-col overflow-x-hidden">
      ...
      <div className="w-full px-4 md:px-8 py-12 mx-auto space-y-12">
        ...
        {/* Bracket Area — DO NOT MODIFY HEIGHT WITHOUT EXPLICIT USER INSTRUCTION */}
