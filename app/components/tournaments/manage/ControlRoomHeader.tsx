...
export default function ControlRoomHeader({ tournament, tournamentId, onBack, onOpenTournament, onStartTournament, onViewBracket }: Props) {
  return (
    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10 mb-16 pb-12 border-b border-white/[0.05]">
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-6">
            <div className="flex items-center gap-3 bg-[#1B1B1B] border border-white/[0.05] px-4 py-2 rounded-xl">
              <div className={`w-2 h-2 rounded-full animate-pulse ${tournament.status === 'COMPLETED' ? 'bg-white/20' : 'bg-primary shadow-[0_0_10px_rgba(82,185,70,0.5)]'}`} />
              <span className="text-[10px] font-black uppercase tracking-widest text-white/60">{tournament.status}</span>
            </div>
            <span className="text-white/20 text-[10px] font-black uppercase tracking-[0.3em] font-poppins">
      ...
      <div className="flex flex-wrap gap-4 w-full lg:w-auto">
        ...
        {tournament.status === "UPCOMING" && (
          <button onClick={onOpenTournament} className="flex-1 lg:px-12 py-5 bg-primary text-black font-black text-[10px] uppercase tracking-[0.3em] rounded-2xl hover:brightness-110 active:scale-95 transition-all shadow-2xl shadow-primary/20 font-poppins">
            Open Registration
          </button>
        )}
        {tournament.status === "OPEN" && (
          <button onClick={onStartTournament} className="flex-1 lg:px-12 py-5 bg-primary text-black font-black text-[10px] uppercase tracking-[0.3em] rounded-2xl hover:brightness-110 active:scale-95 transition-all shadow-2xl shadow-primary/20 font-poppins">
            Start Tournament
          </button>
        )}
      </div>
